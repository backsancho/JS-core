...
$(function () {
  ...
  function puzzleFill() {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      $(`.pzzl:eq(${i})`).attr("value", `${position + 1}`);
      $(`.game-start>.puzzle-box:eq(${i})`).append($(`.pzzl:eq(${i})`));
      check[position] = 1;
    }
    $(".pzzl").css("background-image", "url(image/logo.jpg)");
  }
  // timerStart()
  function timerStart() {
  ...
  function gameCheck() {
    ...
    let checkResult = 0;
    for (let i = 0; i < 16; i++) {
      if ($(`.game-end>.puzzle-box:eq(${i})>.pzzl`).attr("value") == i + 1) {
        checkResult++;
      }
    }
